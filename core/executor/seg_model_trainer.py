...
class SegModelTrainer3D(object):
    ...
    def load_inference_for_feature_analysis(self):
        ...
        fa = feature_analyzer.FeatureAnalyzer(extracted_csv_path=os.path.join(feature_analyze_path,
                                                                              "extracted_features_{}.csv".format(self.args.feature_pattern)),
    ...
    def load_inference_with_gt(self):
        print("[x] restore seg_model ...")
        self.saver.restore(self.sess, os.path.join(self.args.exp, 'seg_model.cpkt'))
        data_gen, val_patients = get_generator(self.args, split=False)

        for idx in tqdm(range(len(val_patients))):
            ...
            model_result = self.evaluate_results(data_y, pred_y, [val_patients[idx]], data_x)
            self.save_result(model_result, epoch=9999)

    def load_inference_with_gt2(self):
        print("[x] restore seg_model ...")
        from core.dataset.common import get_patients
        self.saver.restore(self.sess, os.path.join(self.args.exp, 'seg_model.cpkt'))
        patients = get_patients(self.args.data_list)
        ...
        for idx in tqdm(range(len(patients))):
            ...
            model_result = self.evaluate_results(data_y, pred_y, [patients[idx]], data_x)
            self.save_result(model_result, epoch=10000)
    ...
    def inference_whole_volume(self, data, interval_z=1):
        ...
        :return:
        """
        inferenced_volume = np.zeros_like(data, dtype=np.float)
