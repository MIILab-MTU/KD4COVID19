...
def get_split_deterministic_stratified(all_keys, classes, fold=0, num_splits=5, random_state=12345):
    all_keys_sorted = np.sort(list(all_keys))
    splits = StratifiedKFold(n_splits=num_splits, shuffle=True, random_state=random_state)
    for i, (train_idx, test_idx) in enumerate(splits.split(all_keys_sorted, classes)):
        if i == fold:
            train_keys = np.array(all_keys_sorted)[train_idx]
            test_keys = np.array(all_keys_sorted)[test_idx]
            break
